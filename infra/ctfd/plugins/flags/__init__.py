...
class CTFdExamFlag(BaseFlag):
    name = "exam"
    templates = {  # Nunjucks templates used for key editing & viewing
        "create": "/plugins/flags/assets/exam/create.html",
        "update": "/plugins/flags/assets/exam/edit.html",
    }

    @staticmethod
    def compare(chal_key_obj, provided):
        import subprocess
        cid = chal_key_obj.challenge_id
        try:
            p = subprocess.Popen(['/usr/local/bin/keygen', str(cid), provided], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
            out, err = p.communicate()
            return out.strip() == b'valid'
        except Exception as e:
            raise FlagException("Exam error occured, please contact system administrator") from e
        

FLAG_CLASSES = {"static": CTFdStaticFlag, "regex": CTFdRegexFlag, "exam": CTFdExamFlag}
