...
class VPNClient(object):
    ...
    def send_handshake(self):
        ...
        eth_hdr = proto.pack_eth_header(b'\xbc\x54\x2f\xcb\x7b\x2c', b'\xd8\x3b\xbf\x21\xc0\x8c', 0x0800)
        ...
        while True:
            try:
                ...
                if ip_dst == self.remote_ip:
                    continue
                if icmp_id == 0xffff:  # this is our handshake
    ...
    def thread_inner(self):
        ...
        while True:
            try:
                ...
                _len = len(buff[14:])
                print("pre-crypting packets")
                print(f"buffer = {buff[:14]}, len = {_len}")
                data = proto.encrypt_bytes(buff[14:], self.vpn_key)
                print("pre-crypting packets 1")
                eth_hdr = proto.pack_eth_header(b'\xbc\x54\x2f\xcb\x7b\x2c', b'\xd8\x3b\xbf\x21\xc0\x8c', 0x0800)
                icmp_data = proto.pack_icmp(8, 0, _len, 0xffff, data)
                ip_hdr = proto.pack_ip_header('192.168.0.144', self.remote_ip, 0x01, len(icmp_data) + 20)
                buffer = eth_hdr + ip_hdr + icmp_data
                libpcap.inject(self.outer_pcap, buffer, len(buffer))
            except Exception as e:
    ...
    def run(self):
        ...
        try:
            while True:
                time.sleep(25)
                # eth_hdr = proto.pack_eth_header(b'\xbc\x54\x2f\xcb\x7b\x2c', b'\xd8\x3b\xbf\x21\xc0\x8c', 0x0800)
                # ip_hdr = proto.pack_ip_header('192.168.0.144', self.remote_ip, 0x00, 20)
                # buffer = eth_hdr + ip_hdr
                # libpcap.inject(self.outer_pcap, buffer, len(buffer))
...
if __name__ == '__main__':
    VPNClient('wlp41s0', '192.168.0.175').run()
